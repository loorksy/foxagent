...
12. [خارطة الطريق](#12-خارطة-الطريق) — الخطة التنفيذية: [DEVELOPMENT_PLAN_AR.md](DEVELOPMENT_PLAN_AR.md)
...
### 12.4 خطة التطوير العملية (مصدر الحقيقة)

الخطة التنفيذية الموحدة — غرفة العمليات + عقد الاستراتيجية + البوتات المتعددة — في:
...
4. DSL بدل أعلام الاختيار + القوالب الخمس قابلة لفك التثبيت
5. حلقة الوكيل: وصف → ≤8 باك تستات → بطاقة شات + `/strategy-lab/jobs/:id`
6. سجل مسح + خط زمني + قاطع مسمّى
7. إحاطة تيليجرام حقيقية + تنبيه stale/خبر
8. تاريخ الباك تست + لوحة ترتيب + `bySession` / `byMonth`
9. أوامر تيليجرام + تشريح won/lost + مهام قبل لندن
10. ترحيل جداول + حد معدّل + نسخة احتياطية

الثوابت داخل الخطة: ذهب فقط · البوابة في الكود · Pause فوق الجميع · لا بوت يضع أوامر · لا كود حر من الوكيل · لا اختلاق أخبار.

أي تعارض بين §12.2 أعلاه وتلك الوثيقة: الوثيقة التنفيذية هي المرجع لدورة البناء هذه.
...
├── docs/DEVELOPMENT_PLAN_AR.md     خطة التطوير — عشر سفن
